...
class CORnet_RT(nn.Module):
    ...
    def forward(self, inp):
        ...
        for block in blocks[1:]:
            if block == 'V1':  # at t=0 input to V1 is the image
                ...
                this_inp = None
            # print(this_inp.shape, len(outputs['inp']))
    ...
    def forward_and_extract(self, inp):
        ...
        for block in blocks[1:]:
            ...
            # print(this_inp.shape, len(outputs['inp']))
            # new_output, new_state = getattr(self, block)(this_inp, batch_size=len(outputs['inp']))
            new_output, new_state, features = getattr(self, block).forward_and_extract(this_inp, batch_size=len(outputs['inp']))
